...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($data->email) && isset($data->password)) {
        ...
        // Priprema SQL upita
        $stmt = $pdo->prepare("SELECT * FROM professors WHERE email = :email");
        ...
        if ($user && password_verify($password, $user['password'])) {
            // Prijava uspešna
            $_SESSION['name'] = $user['name']; // Dodavanje imena profesora u sesiju
            echo json_encode(["success" => true, "message" => "Prijava uspešna.", "name" => $user['name']]);
        } else {
            // Neuspešna prijava
            echo json_encode(["success" => false, "message" => "Neispravni email ili lozinka."]);
        }
    } else {
        // Nedostaju podaci
